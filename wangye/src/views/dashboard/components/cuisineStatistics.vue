<template>
  <section class="product-summary cm-surface">
    <header>
      <span>菜品</span>
      <router-link to="/dish">管理</router-link>
    </header>
    <strong>{{ total }}</strong>
    <p>当前菜品总数</p>
    <div class="product-summary__status">
      <span><i class="is-on" />启售 {{ number(dishesData.sold) }}</span>
      <span><i />停售 {{ number(dishesData.discontinued) }}</span>
    </div>
    <router-link class="product-summary__add" to="/dish/add">+ 新增菜品</router-link>
  </section>
</template>
...
@Component({ name: 'CuisineStatistics' })
export default class extends Vue {
  @Prop({ default: () => ({}) }) private readonly dishesData!: any

  get total() {
    return this.number(Number(this.dishesData.sold || 0) + Number(this.dishesData.discontinued || 0))
  }
